...
Add WebUI to your Deno project via [JSR](https://jsr.io/@webui/deno-webui):

```sh
deno add jsr:@webui/deno-webui
```
...
Run your app with all required permissions:

```sh
deno run --allow-read --allow-write --allow-net --allow-env --allow-ffi app.ts
```
...
const win = new WebUI();
await win.show('<html><script src="webui.js"></script> Hello World! </html>');
await WebUI.wait();
```
...
Bind a Deno callback to an HTML element by its `id`. When the element is clicked (or the matching function is called from JavaScript), the callback is invoked with a [`WebUI.Event`](#event).
...
| `e.arg.number(index)` | `number` | Get a number argument passed from JS |
...
myWindow.bind('myFunc', (e: WebUI.Event) => {
  ...
  const checked = e.arg.boolean(2);
  return `received: ${text}, ${value}, ${checked}`;
});
```
...
myWindow.run('alert("Hello from Deno!")');
```
...
Intercept the WebView close button. Return `false` from the handler to prevent the window from closing (e.g. to show a "are you sure?" dialog first).

```ts
myWindow.setCloseHandlerWv((win: WebUI) => {
...
Get the unique numeric ID assigned to this window by WebUI.
...
Get the current WebUI Deno wrapper version string.

```ts
console.log(WebUI.version); // e.g. "2.5.13"
```
