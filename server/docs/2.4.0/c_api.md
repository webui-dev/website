...
- Window
    ...
    - [Embedded Icon](/c_api?id=embedded-icon)
- Binding & Events
    - [Bind](/c_api?id=Bind)
    - [Events](/c_api?id=events)
    - [Files Handler](/c_api?id=files-handler)
- Application
    - [Wait](/c_api?id=wait)
    - [Exit](/c_api?id=exit)
...
To embed an icon (_String format_) whtin the application, you can use `webui_set_icon()`.
...
You can use `webui_set_file_handler()` to generate files dynamicaly. This become handy when you would like to create REST APIs for your UI.
...
webui_set_file_handler(my_window, my_files_handler);
```
